...
Foreman manages GitHub for Emotion-SRL through a `justfile` that wraps the `gh` CLI. Always use the recipes in this skill's `justfile` instead of calling `gh` directly — the justfile handles the SVILUPPO project configuration, status option IDs, and cross-repo iteration so you don't have to.
