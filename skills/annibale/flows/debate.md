...
```
[1. CHRISTOPHER]    → retrieves context on the topic from the TB
...
[5. CHRISTOPHER]    → verifies the updated connections
→ back to [2] or close
```
...
```bash
just run <member> "Use the christopher skill to retrieve everything the Third Brain knows about: <topic>"
```
...
```bash
just run <member> "Use the socrate skill.
...
```bash
just run <member> "Use the aristotele skill.
...
```bash
just run <member> "Use the christopher skill to re-verify the Third Brain graph on: <topic> — show what has changed since the start of the cycle."
```
