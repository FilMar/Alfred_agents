...
- Pass explicit context: every `th run` must receive a task containing everything the agent cannot know on its own. Keep task text plain: no backticks, no `$()`, no double quotes.
