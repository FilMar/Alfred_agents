...
A skill is executed **inline** by reading `SKILL.md` and applying it. A `th` member is a separate agent invoked only by orchestrator skills (`annibale`).
...
2. **Aggregate into benchmark** — use this skill's justfile:
   ```bash
   just -f skills/efesto/justfile aggregate <workspace> <iteration> <skill-name>
   ```
...
4. **Launch the viewer** with both qualitative outputs and quantitative data:
   ```bash
   just -f skills/efesto/justfile review <workspace> <iteration> <skill-name> [<previous-iteration>]
   ```
...
```bash
just -f skills/efesto/justfile optimize-description <path-to-trigger-eval.json> <path-to-skill> <model-id> --max-iterations 5 --verbose
```
...
The `--model` argument uses pi's format: `provider/model-id` (e.g. `anthropic/claude-sonnet-4-6`). Test runs are sandboxed via `just -f skills/efesto/justfile test-skill <path>`.
...
```bash
just -f skills/efesto/justfile package <path/to/skill-folder> [<output-directory>]
```
