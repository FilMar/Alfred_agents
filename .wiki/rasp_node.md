...
Target stack, in dependency order. Status per step below.

1. Tailscale + ACL tags (perimeter first — nothing else should come up before it)
2. conduwuit container (Matrix homeserver)
3. **Done (2026-09-11)** — Qdrant + Ollama containers, live and confirmed working: `tb` and `ti` both run against the Rasp instance, from desktop and laptop. See [tb_on_rasp](tb_on_rasp).
...
5. Matrix bot (shared plumbing for orchestrator commands and `!tb search` — see [roadmap](roadmap), rasp area)
...
- [roadmap](roadmap) — rasp area: the personal server task
