...
Both are **evaluative/scoring layers only**. They measure how well a member/hat performs; they do not extract, store, or revise reusable procedures. They also do not touch the skills system (`.claude/skills/*.md`) at all — skills are a separate, disconnected mechanism, authored manually (e.g. via the Blacksmith skill).

## The six gaps

In dependency order — each presupposes the data or mechanism from the one above it.

1. **No telemetry on skills.** `th.db` tracks execution + score per `th` member/hat (Phase 2C/7). Skills (scribe, gardener, courier, blacksmith, etc.) have no equivalent: no log of invocation frequency, context, or outcome. The measurement side is only half-covered — `th` members yes, skills no.
...
4. **No revision/versioning loop.** `tb` is purely additive (notes + refs); skills only update when someone (Blacksmith) manually rewrites them. No mechanism exists that, given accumulated evidence, automatically updates or deprecates an existing procedure.
...
**Next concrete step** when work starts: schema of the `events` table + the skill-invocation hook, to be founded as a new module via Architect (project governance).
