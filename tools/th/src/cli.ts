...
import { listAvailableModels, listAvailableSkills, runMember, spawnDetached, tryReexecWithBwrap } from "./runner.js";
...
member
  ...
  .option("--from <global>", "Crea da un membro globale come base (ignora --hat, --role, --tools, --skills)")
  .action((name: string, opts) => {
    try {
      if (opts.from) {
        ...
        out(createMember(name, opts.hat, opts.role, splitCSV(opts.tools), splitCSV(opts.skills), opts.tmp));
      }
    } catch (err) {
...
program
  .command("skills")
  .description("Lista skill disponibili nel progetto corrente")
  .action(async () => {
    try {
      const skills = await listAvailableSkills();
      if (skills.length === 0) die("Nessuna skill trovata.");
      out(skills);
    } catch (err) {
      die(errorMessage(err));
    }
